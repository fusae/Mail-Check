...
如需支持“按钮反馈 + 自然语言回复”，请使用企业微信应用消息模式：

1. 配置 `config/config.yaml` 中的企业微信应用信息（corp_id/agent_id/secret/token/encoding_aes_key）。
2. 在企业微信后台配置回调 URL（示例）：`https://你的域名/wechat/callback`。
3. 启动回调服务：

```bash
source venv/bin/activate
python3 src/wechat_webhook_server.py
```

说明：
- `to_user`/`to_party`/`to_tag` 用于指定接收人。
- 点击“误判舆情”后会删除对应负面舆情记录，并写入反馈表。
