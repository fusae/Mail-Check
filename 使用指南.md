...
群机器人无法回调时，可用“反馈链接”收集误报/确认：

1. 在 `config/config.yaml` 填写：
   - `feedback.link_base_url`：如 `https://你的域名/feedback`
   - `feedback.link_secret`：用于签名
2. 启动反馈服务：
...
说明：
- 消息里会自动附反馈链接，打开后可选“误判/确认”并填写说明。
- 误判会删除对应负面舆情记录，并写入反馈表与规则表。
