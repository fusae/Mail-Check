...
```bash
# 使用nohup后台运行
nohup bash start_all.sh > output.log 2>&1 &
...
2026-01-14 16:50:05 - email_monitor - INFO - 成功连接到邮箱: 1103560723@qq.com
2026-01-14 16:50:06 - email_monitor - INFO - 找到 0 封新邮件
2026-01-14 16:50:06 - email_monitor - INFO - 已断开邮箱连接
...
```yaml
feedback:
  link_base_url: "http://<服务器地址>:<API端口>/feedback"  # 反馈地址（端口以配置为准）
...
sentiment_id = 'test_123456'
sig = hmac.new(secret.encode(), f'{sentiment_id}'.encode(), hashlib.sha256).hexdigest()
print(f'http://<服务器地址>:<API端口>/feedback?sentiment_id={sentiment_id}&sig={sig}')
"
```
